...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    #row == list of variables in each row
    for row in var_array:
        if model == 'neq':
            constraint_list.extend(post_all_pairs(row))
        elif model == 'alldiff':
            constraint_list.append(AllDiffConstraint('allDiffRow {}'.format(row), row))

    for colj in range(len(var_array[0])):
        #scope = list of colvariables
        scope = list(map(lambda row: row[colj], var_array))
        if model == 'neq':
            constraint_list.extend(post_all_pairs(scope))
        elif model == 'alldiff':
            constraint_list.append(AllDiffConstraint('allDiffCol {}'.format(colj), scope))
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(AllDiffConstraint('allDiffSqr {}{}'.format(i//3,j//3), scope))
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    for solution in solutions:
        ...
        for i in range(len(planes_problem.planes)):
            sol.append([planes_problem.planes[i]])
            
        for (var, val) in solution:
            plane = int(var.name()[6])
            if val != 'NF':
...
def planeCSP(planes_problem):
    ...
    3.  The sequence of flights flown by every plane must be feasible. That is, if F2 follows F1 in a plane’s
         sequence of flights then it must be that F2 can follow F1 (normally this would mean that F1 arrives at
         the same location that F2 departs).
    4.  Certain flights terminate at a maintenance location. All planes must be serviced with a certainminimum
         frequency. If this minimum frequency is K then in the sequence of flights assigned to aplane at least
         one out every subsequence of K flights must be a flight terminating at a maintenance location.
         Note that if a plane is only assigned J flights withJ<K, then it satisfies this constraint.
    5.  Each flight must be scheduled (be part of some plane’s sequence of flights). And no flight can be scheduled
        more than once.
    """
    # important variables from planes
    ...
    can_follow_map = {}
    for (f1,f2) in can_follow:
        can_follow_map.setdefault(f2, []).append(f1)
    ...
    for plane_id, plane in enumerate(planes):
        ...
        for seq_num in range(len(can_fly)):
            dom = []
            if seq_num == 0: #first flight
                ...
                dom.extend(can_fly)
                dom.append("NF")
            var = Variable("Plane {},{}".format(plane_id,seq_num), dom)
            var_array[plane_id].append(var) #a list of variables for 1 Plane
            #[Plane1_0, Plane1_1, Plane1_2], [Plane2_0, Plane2_1, Plane2_2]
    
    constraint_list = []

    #C3: 
    for plane in var_array:
        constraint_list.append(FlightFollowsConstraint("flightFollow {}".format(plane), plane, can_follow_map))
    
    #C4:
    required_values = ["NF"]
    required_values.extend(maintenance_flights)
    for plane in var_array:
        for j in range(len(plane) - min_maintenance_frequency + 1):
            scope = plane[j:j+min_maintenance_frequency]
            constraint_list.append(NValuesConstraint("maintenance {}".format(scope),scope, required_values, 1, min_maintenance_frequency))

    #C5:
    vars = [var for row in var_array for var in row]
    constraint_list.append(AllFlightsConstraint("allFlights", vars, flights))

    # constraint_list.append(NotEnoughtMaintConstraint("notEnoughtMaint", vars, maintenance_flights, min_maintenance_frequency ))
